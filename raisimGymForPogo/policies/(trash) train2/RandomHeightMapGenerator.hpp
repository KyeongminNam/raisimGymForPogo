...
namespace raisim {
    ...
    public:
        ...
        raisim::HeightMap* generateTerrain(raisim::World* world,
            ...
            const double maxAngle = 20. * (M_PI / 180.);
            const double slopeTargetAngle = 20. * (M_PI / 180.);
            ...
            switch (groundType) {
                ...
                case GroundType::INCLINED_STEPS: {
                    width_ = 0.2 + 0.3 * uniDist(gen);
                    height_ = 0.15 * curriculumFactor;
                    ...
                    for (int xBlock = 0; xBlock < nBlock_; ++xBlock) {
                        for (int yBlock = 0; yBlock < nBlock_; ++yBlock) {
                            ...
                            for (int i = 0; i < nGridPerBlock_; ++i) {
                                for (int j = 0; j < nGridPerBlock_; ++j) {
                                    heightVec[nGrid_ * (nGridPerBlock_ * yBlock + j) + (nGridPerBlock_ * xBlock + i)] =
                                            stepHeight + yBlock * height_;
                                }
                            }
                        }
                    }
                ...
                case GroundType::INCLINED_STEPS2: {
                    width_ = 0.2 + 0.3 * uniDist(gen);
                    height_ = 0.3 * curriculumFactor;
                    width_ = std::max(width_, height_ / std::tan(maxAngle));
                ...
                case GroundType::STAIRS2: {
                    width_ = 0.1 + 0.4 * uniDist(gen);
                    double maxHeight = 0.3;

                    double p = uniDist(gen);
                    if (p < 0.5) height_ = std::abs(maxHeight * curriculumFactor);
                    else height_ = std::abs(maxHeight * curriculumFactor * (1. + uniDist(gen)) / 2.);
