...
import raisimGymForPogo.algo.ppo.ppo as PPO
...
saver = ConfigurationSaver(log_dir=home_path + "/raisimGymForPogo/data/" + task_name,
                           save_items=[task_path + "/cfg.yaml", task_path + "/Environment.hpp", task_path + "/PogoController.hpp",
                                       task_path + "/RandomHeightMapGenerator.hpp", task_path + "/runner_MLP2.py",
...
ppo = PPO.PPO(actor=actor,
              ...
              lam=0.95,
              num_mini_batches=1,
              policy_learning_rate=4e-3,
              value_learning_rate=4e-3,
              lr_scheduler_rate=0.9985,
              max_grad_norm=0.5,
              device=device,
...
for update in range(iteration_number, 30001):
    ...
    # if update % cfg['environment']['eval_every_n'] == 0:
        ...
        data_std = np.sqrt(np.clip(data_square_sum - data_size * data_mean**2, 1e-16, 1e16) / (data_size - 1 + 1e-16))
