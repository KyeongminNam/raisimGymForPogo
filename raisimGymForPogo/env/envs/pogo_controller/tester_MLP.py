...
def map_axis(axis_value):
    if -0.1 < axis_value and axis_value < 0.1:
        return 0
    elif 0.1 <= axis_value and axis_value < 0.55:
        return 1
    elif 0.55 <= axis_value:
        return 2
    elif -0.55 < axis_value and axis_value <= -0.1:
        return -1
...
if weight_path == "":
    ...
    loaded_graph = ppo_module.MLP(cfg['architecture']['mlp3_shape'], torch.nn.LeakyReLU, ob_dim, act_dim)
    ...
    try:
        ...
        while running:
            ...
            if len(joysticks) > 0:
                command[0] = map_axis(joysticks[0].get_axis(1))
                command[1] = map_axis(joysticks[0].get_axis(0))
                command[2] = map_axis(joysticks[0].get_axis(3))
            print(command)
            env.set_command(command)
            obs = env.observe(False)
            with torch.no_grad():
                action_ll = loaded_graph.architecture(torch.from_numpy(obs).cpu())
                reward_ll, dones = env.step(action_ll.cpu().detach().numpy(), 1.0)
        ...
        ax0.plot(infoBag_np[:, 0], label='R1', color='tab:red')
        ax0.plot(infoBag_np[:, 1], label='R2', color='tab:green')
        ax0.plot(infoBag_np[:, 2], label='P', color='tab:blue')
        ax0.plot(infoBag_np[:, 0], label='R1', linestyle='dashed', color='tab:red')
        ax0.plot(infoBag_np[:, 1], label='R2', linestyle='dashed', color='tab:green')
        ax0.plot(infoBag_np[:, 2], label='P', linestyle='dashed', color='tab:blue')
        ax0.set_title('GC')
